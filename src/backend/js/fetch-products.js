async function fetchProducts() {
  ...
  try {
    let response = await fetch("backend/fetch-products.php");
...
function displayproducts(products) {
  ...
  if (!Array.isArray(products) || products.length === 0) {
    tableBody.innerHTML = `<tr><td colspan="8" class="text-center text-gray-400">No products found</td></tr>`;
    return;
  }
    ...
    .map((product, index) => {
      return `
          <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-200 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
            <th scope="row" class="px-6 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">${product.name}</th>
