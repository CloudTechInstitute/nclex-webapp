  ...
  .addEventListener("submit", async function (e) {
    ...
    try {
      ...
      if (response.ok && result.status === "success") {
        ...
        showToast(toastMessage, toastType, true); // Pass `true` to indicate redirection after
      } else {
        toastMessage = result.message || "User not found";
        toastType = "error";
...
function showToast(message, type, shouldRedirect = false) {
  ...
  setTimeout(() => {
    toast.remove();
    if (shouldRedirect) {
      window.location.href = "index.php"; // Redirect only after toast disappears
    }
  }, 3000);
}
